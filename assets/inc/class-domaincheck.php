...
class WP24_Domain_Check_Domaincheck {
	...
	private function whois( $domain, $tld, $whoisserver, $status_free ) {
		...
		if ( '' == $string || preg_match( '/whois_query_error/', $string ) ) {
			...
			// if status free start with "-" do negated match
			if ( '-' != substr($status_free, 0, 1) && preg_match( '/' . $status_free . '/i', preg_replace( '/\s\s+|\t/', ' ', $string ) ) )
			...
			else if ( '-' == substr( $status_free, 0, 1 ) && ! preg_match( '/' . substr( $status_free, 1 ) . '/i', preg_replace( '/\s\s+|\t/', ' ', $string ) ) )
				$array_result['status'] = 'available';
			else {
